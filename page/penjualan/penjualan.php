   ...
   if(isset($_POST['simpan'])){
       ...
       $barang =  $koneksi->query("select * from tb_penjualan_detail where kode_penjualan='$kode'");
         ...
         while ($data_madu = $madu->fetch_assoc()) {
               ...
               $koneksi->query("UPDATE tb_barang SET stok=stok -'$data_madu[jumlah]' WHERE kode_barcode='$data_madu[kode_barcode]'");
     ...
     if(kode_barcode != "")
     {
       ...
       axios.get("page/penjualan/ambil-detail-barang.php?kode_barcode=" + kode_barcode)
         .then(function(res){
           ...
           {
             document.getElementsByName("total_bayar")[0].value = detail_madu.harga_jual;
           }
           hitungSubTotal(detail_madu);
         })
     }
     
   }
   function hitungSubTotal(detail_madu_sekarang)
   {
     var jumlah = parseInt(document.getElementById('jumlah').value);
     detail_madu_sekarang.stok = parseInt(detail_madu.stok);

     // cek jumlah yang diketik apakah dibawah stok atau tidak
     if(jumlah > detail_madu_sekarang.stok)
     {
        alert("Jumlah madu melebihi stok");
        document.getElementById('jumlah').value = 0;  
     }
     else if(jumlah == 0)
     {
      alert("Jumlah madu tidak boleh kosong");
